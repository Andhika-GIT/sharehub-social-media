...
// custom active or not active style
const isNotActiveStyle = 'flex items-center px-5 gap-3 text-gray-500 hover:text-black transition-all duration-200 ease-in-out capitalize';
const isActiveStyle = 'flex items-center px-5 gap-3 font-extrabold border-r-2 border-black transition-all duration-200 ease-in-out capitalize';

// prepare categories for sanity
const categories = [{ name: 'Animals' }, { name: 'Wallpapers' }, { name: 'Photography' }, { name: 'Gaming' }, { name: 'Coding' }];

const Sidebar = ({ user, closeToggle }) => {
  const handleCloseSidebar = () => {
    // if closeToggle is true (based on the toggleSidebar state in home.jsx)
    if (closeToggle) {
      // then run closeToggle method to set the toggleSidebar state in home.jsx to false
      closeToggle(false);
    }
  };

  return (
    <div className="fle flex-col justify-between bg-white h-full overflow-y-scroll min-w-210 hide-scrollbar">
